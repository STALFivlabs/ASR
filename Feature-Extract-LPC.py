...
##Working on LPC Algorithms for Feature Extraction
Importing Libraries
"""
...
def lpc(signal, sampling_freq, no_coeff):
  ...
  no_sample = (0.025*sampling_freq)
  no_delay = (0.010*sampling_freq)
  no_frame = math.ceil(len(signal)/(no_sample-no_delay))

  #Padding
  padding = ((no_sample-no_delay)*no_frame) - len(signal)
  if padding > 0:
    s = np.append(signal, np.zeros(padding), axis=1)
  else:
    s = signal
  print('Stage1: Done')
